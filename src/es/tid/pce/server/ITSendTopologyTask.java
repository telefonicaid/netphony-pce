 ...
 *
 */
public class ITSendTopologyTask extends TimerTask {
    ...
    public void run() {
    	...
    	Hashtable<StorageTLV, Object> StorageAndSite = ((SimpleITTEDB)tedb).getStorageCharacteristics(); //potencialmente CORREGIBLE
    	
    	Enumeration <StorageTLV> storages = StorageAndSite.keys();
    	while (storages.hasMoreElements()){
        	...
        	Notification notification=new Notification();
        	//3.1.- Set Notification type = PCEP_NOTIFICATION_TYPE_TOPOLOGY=101
        	notification.setNotificationType(ObjectParameters.PCEP_NOTIFICATION_TYPE_IT_RESOURCE_INFORMATION);
        	//3.2.- Set Notification value = PCEP_NOTIFICATION_VALUE_UPDATE=0x01
        	notification.setNotificationValue(ObjectParameters.PCEP_NOTIFICATION_VALUE_UPDATE);
        	//7.- Add the TLV and the notification object
        	ITAdvertisementTLV ITadvtlv = new ITAdvertisementTLV();
        	ITadvtlv.setAdv_ID((Inet4Address)StorageAndSite.get(storage));
        	ITadvtlv.setAdv_Type(0x01);
        	ITadvtlv.setVirtual_IT_Site_ID((Inet4Address)StorageAndSite.get(storage));
        	notification.setITadvtlv(ITadvtlv);
        	notification.setStoragetlv(storage);
        	log.info("UnitaryPrice:"+(storage.costList.getFirst().getUnitaryPrice()).toString());
        	notificationList.add(notification);
        	notify.setNotificationList(notificationList);
        	notificationMessage.addNotify(notify);
        	//8.- Send the notification to the PCE parent

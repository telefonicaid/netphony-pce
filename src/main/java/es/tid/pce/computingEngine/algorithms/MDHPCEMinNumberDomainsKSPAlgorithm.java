...
public class MDHPCEMinNumberDomainsKSPAlgorithm implements ComputingAlgorithm {
	...
	public MDHPCEMinNumberDomainsKSPAlgorithm(ComputingRequest pathReq, TEDB ted, ChildPCERequestManager cprm,
	...
	public ComputingResponse call() throws Exception {
		...
		if (EP.getOT()==ObjectParameters.PCEP_OBJECT_TYPE_ENDPOINTS_IPV4){
			...
			if(gep.getGeneralizedEndPointsType()==ObjectParameters.PCEP_GENERALIZED_END_POINTS_TYPE_P2P){
				...
				source_router_id_addr =
						((IPv4AddressEndPoint)p2pep.getSourceEndpoint().getEndPoint()).getEndPointIPv4().getIPv4address();
				dest_router_id_addr =
						((IPv4AddressEndPoint)p2pep.getDestinationEndpoint().getEndPoint()).getEndPointIPv4().getIPv4address();
			}
		...
		//DijkstraShortestPath<Inet4Address,InterDomainEdge> dsp=new
		// DijkstraShortestPath<Inet4Address,InterDomainEdge> (networkGraph,
		...
		List<GraphPath<Inet4Address,InterDomainEdge>>
		gps=dsp.getPaths(dest_domain_id);
		if (gps==null){
		...
		while((pathfound==false) && (gps.get(k)!=null)) {
			...
			log.info("First part of the LSP is in domain: "+ domain+" from "+
					source_router_id_addr+" to "+destIP);
			//FIXME: METRICA? OF? BW?
			long requestID;
			...
			for (i=1;i<edge_list.size();++i){
				...
				if (EP.getOT()==ObjectParameters.PCEP_OBJECT_TYPE_GENERALIZED_ENDPOINTS){
					GeneralizedEndPoints gep2=(GeneralizedEndPoints) req.getEndPoints();

					if(gep2.getGeneralizedEndPointsType()==ObjectParameters.PCEP_GENERALIZED_END_POINTS_TYPE_P2P){
						EndPointIPv4TLV sourceIPv4TLV = new EndPointIPv4TLV();
						EndPointIPv4TLV destIPv4TLV = new EndPointIPv4TLV();
						...
						((P2PGeneralizedEndPoints)gep2).setSourceEndpoint(sourceEPA);
						((P2PGeneralizedEndPoints)gep2).setDestinationEndpoint(destEPA);
			...
			log.info("Last part of the LSP is in domain: "+ Last_domain+" from "+
					last_source_IP+" to "+dest_router_id_addr);
			...
			//
			//
			// //Preguntar a si esto tiene lógica...
			// //m_resp.addResponse(response);
				...
				for (i=0;i<respList.size();++i){
					if (respList.get(i)==null){
						...
						if(respList.get(i).getResponse(0).getNoPath()!=null){
							...
							ExplicitRouteObject eroInternal
							=respList.get(i).getResponse(0).getPath(0).getEro();
							log.info(" eroInternal "+eroInternal.toString());
							ero.addEROSubobjectList(eroInternal.EROSubobjectList);
							UnnumberIfIDEROSubobject unnumberIfDEROSubobj = new
	...
	public void processXRO(ExcludeRouteObject xro,
			DirectedWeightedMultigraph<Inet4Address, InterDomainEdge> networkGraph) {
		if (xro != null) {
					...
					if (hasVertex) {
						...
						while (iter.hasNext()) {
							...
							if (edge.getSrc_if_id() == eros.getInterfaceID()) {
								...
								// InterDomainEdge edge2=networkGraph.getEdge(edge.getDst_router_id(),
								// edge.getSrc_router_id());
							}
						}

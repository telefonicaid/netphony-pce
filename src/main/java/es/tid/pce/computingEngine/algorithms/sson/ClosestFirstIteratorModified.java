...
public class ClosestFirstIteratorModified<V, E>
    extends CrossComponentIteratorModified<V,
        E, FibonacciHeapNode<ClosestFirstIteratorModified.QueueEntry<V, E>>>
{
    //~ Instance fields --------------------------------------------------------
     ...
     * graph. Iteration will start at the specified start vertex and will be
     * limited to the subset of the connected components which have enough free 
     * spectrum across the links which traverse the iterator.
     * The specified start vertex may not be <code>null</code>.
     *
     * @param g the graph to be iterated.
     * @param startVertex the vertex iteration to be started.
     * @param radius limit on path length, or Double.POSITIVE_INFINITY for
    ...
    protected boolean encounterVertexAgain(V vertex, E edge)
    {
        ...
        // If the seen node has no available channels in its label we overwrite the label with the new
        // bitmapLabelState and actualized the spanning tree.
        if(super.vertexSpectrumState.get(vertex).getSumaBits()==0){

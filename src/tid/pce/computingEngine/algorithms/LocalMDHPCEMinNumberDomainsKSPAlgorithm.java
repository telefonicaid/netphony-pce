...
public class LocalMDHPCEMinNumberDomainsKSPAlgorithm implements ComputingAlgorithm{
	...
	public ComputingResponse call()throws Exception{
		...
		List<GraphPath<Inet4Address,InterDomainEdge>> gps=null;
		...
		*/
		...
		if (EP.getOT()==ObjectParameters.PCEP_OBJECT_TYPE_ENDPOINTS_IPV4){			
			...
			((EndPointsIPv4) endpointsRequest).setDestIP(dest_router_id_addr);
		...
		Inet4Address domain =dest_domain_id;
		log.info("First part of the LSP is in domain: "+ domain+" from "+ source_router_id_addr+" to "+dest_router_id_addr);
...
//		}
		...
		/*if (first_domain_equal==true){
			...
			idLink.setRouterID(edge_list.get(0).getSrc_router_id());
				...
				if(respList.get(i).getResponseList().get(0).getNoPath()!=null){
					...
					if (edge_list != null){
						if (j<edge_list.size()){						
							...
							unnumberIfDEROSubobj.setRouterID(edge_list.get(j).getSrc_router_id());
